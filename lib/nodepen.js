 ...
 *
 * e.g getPens('popular', null, 4, 2) or getPens('user', 'mallendeo', 1, 2)
 *
 * @param {String} source
 * @param {String} name
 * @param {Int} start
  ...
  var getPens = function () {
    ...
    request(pageUrl, function (error, response, body) {
      ...
      $('.single-pen').each(function() {
        ...
        pen.user = $(this).find('.user a').attr('href').replace('/','');
        ...
        pen.loves = parseInt(loves.children('.count').text().trim());
